...
void main() {
  ...
  Future<void> launch(WidgetTester tester) async {
    ...
    await tester.pumpWidget(UncontrolledProviderScope(container: container, child: const BrainLandApp()));
    await tester.pumpAndSettle();
  }

  Future<void> tapKey(WidgetTester tester, String key, {int settleMs = 400}) async {
  ...
  testWidgets('a mission in every world, with mistakes, hints, pause and restart', (tester) async {
    ...
    final firstWrong = math.first.options.keys.firstWhere((v) => v != math.first.answer);
    ...
    await tester.pumpAndSettle();
    final wrong = logic.first.choices.indexWhere((c) => c != logic.first.answer);
    ...
    await tester.pumpAndSettle();
    final cards = find.byWidgetPredicate((wd) => wd.key is ValueKey<String> && (wd.key! as ValueKey<String>).value.startsWith('card-'));
    final keys = [for (final e in cards.evaluate()) (e.widget.key! as ValueKey<String>).value];
    ...
    for (final pair in byFace.values) {
      ...
      if (pair == byFace.values.elementAt(byFace.length - 2)) await shot(tester, '12_memory_progress');
    }
    await next(tester);
    ...
    for (var b = 0; b < boards.length; b++) {
      ...
      if (b == 0) {
        final wrongSlot = List.generate(board.slots.length, (i) => i).firstWhere((s) => !board.pieces[0].fits(board.slots[s]));
        await tapKey(tester, 'piece-0');
        await tapKey(tester, 'slot-$wrongSlot');
      }
      for (var p = 0; p < board.pieces.length; p++) {
        final s = List.generate(board.slots.length, (i) => i).firstWhere((s) => board.pieces[p].fits(board.slots[s]));
    ...
    final before = {for (final w in World.values) w: (store.get('stars.${w.slug}') as List).first};
    await store.close();
    store = await Hive.openBox<dynamic>(kStoreBox);
